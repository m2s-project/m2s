...
const CommandeForm = () => {
  ...
  useEffect(() => {
    state.item && setCommande(state.item);
  }, [state]);
...
switch (state.type) {
  case 'new':
    dispatch(upsertCommande({ id: uuid(), data:commande }))
    break;
  case 'delete':
    dispatch(deleteCommande(commande.id))
    navigate(-1)
    break;
  case 'update':
    dispatch(upsertCommande({ id: commande.id, data:commande }))
    navigate(-1)
    break;
